...
class SkipCells(nbconvert.preprocessors.Preprocessor):
    """Hide notebook code cells in generated output."""
...
def convert_notebook(name, force):
    ...
    nbconvert_version = subprocess.check_output(["jupyter", "nbconvert", "--version"]).decode("utf-8").strip()
    if nbconvert_version not in ["4.0.0", "4.1.0", "4.2.0", "5.2.1"]:
    ...
    with open(source) as f:
        ...
        # without error.
        execute = nbconvert.preprocessors.ExecutePreprocessor(timeout=600)
...
if os.path.exists(build_dir):
    for path in os.listdir(build_dir):
        for subpath in os.listdir(os.path.join(build_dir, path)):
            if os.path.isdir(os.path.join(build_dir, path, subpath)):
                shutil.rmtree(os.path.join(build_dir, path, subpath))
            else:
                os.remove(os.path.join(build_dir, path, subpath))
